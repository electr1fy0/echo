import { desc, eq, and, inArray, notInArray, sql } from "drizzle-orm";
import type { DB } from "../db";
import { schema } from "../db";

export const createNotification = async (db: DB, payload: {
  userUsername: string;
...
export const countUnread = async (db: DB, currentUser: string): Promise<number> => {
...
export const markAllAsRead = async (db: DB, currentUser: string) => {
  await db
...
export const listNotifications = async (db: DB, currentUser: string, limit: number, offset: number) => {
  ...
  for (const row of rows) {
    if (["upvote_post", "mention_post", "express_interest", "milestone"].includes(row.type)) {
  ...
  const [posts, replies] = await Promise.all([
    postRefUids.size
      ? db.select({ uid: schema.posts.uid, slug: schema.posts.slug, content: schema.posts.content }).from(schema.posts).where(inArray(schema.posts.uid, [...postRefUids]))
      : Promise.resolve([] as { uid: string; slug: string; content: string | null }[]),
    replyRefUids.size
      ? db.select({ uid: schema.replies.uid, content: schema.replies.content, postUid: schema.replies.postUid }).from(schema.replies).where(inArray(schema.replies.uid, [...replyRefUids]))
  ...
  const parentPosts = replyPostUids.size
    ? await db.select({ uid: schema.posts.uid, slug: schema.posts.slug, content: schema.posts.content }).from(schema.posts).where(inArray(schema.posts.uid, [...replyPostUids]))
    : [];
...
export const cleanupOrphaned = async (db: DB, currentUser: string) => {
  await db
    ...
    .where(
      and(
        ...
        notInArray(schema.notifications.referenceUid, db.select({ uid: schema.posts.uid }).from(schema.posts)),
      ),
    );
  ...
  await db
    ...
    .where(
      and(
        ...
        notInArray(schema.notifications.referenceUid, db.select({ uid: schema.replies.uid }).from(schema.replies)),
      ),
    );
};
