...
import { useMessages, useSendMessage, useConversations, useMarkConversationRead } from "@/hooks/use-dms";
        ...
        {isLoading ? (
          <div className="space-y-3 p-4">
            {Array.from({ length: 4 }).map((_, i) => (
              ...
              >
                <Skeleton
                  className={`h-10 w-3/4 rounded-2xl ${i % 2 === 0 ? "rounded-bl-sm" : "rounded-br-sm"}`}
                />
              </div>
            ))}
          </div>
              ...
              return (
                ...
                >
                  <div className="max-w-[80%] flex flex-col">
                    ...
                    <span
                      className={`text-[10px] text-neutral-400 mt-0.5 ${isMine ? "text-right mr-1" : "ml-1"}`}
                    >
                      {formatDistanceToNowStrict(new Date(msg.timeCreated), {
                        addSuffix: true,
                      })}
                    </span>
                  </div>
