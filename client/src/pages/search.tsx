...
import { useSearchQuestions, useDeleteQuestion } from "@/hooks/use-questions";

export function Search() {
  const [query, setQuery] = useState("");

  const { data: results = [], isLoading, error } = useSearchQuestions(query);
    ...
    <div className="max-w-xl w-full mt-40 space-y-8 mb-40 relative px-4 pb-20 md:pb-0">
      ...
      <div className="space-y-4">
        {query ? (
          <>
            {isLoading ? (
              <p className="text-neutral-500 text-sm px-1">Searching...</p>
            ) : error ? (
              <p className="text-red-500 text-sm px-1">Failed to search</p>
            ) : (
              <>
