...
export function DateTimePicker({ value, onChange, placeholder, className }: DateTimePickerProps) {
  ...
  const commitTime = React.useCallback((h: number, m: number, mer: string) => {
      ...
      >
          ...
          {currentView === "date" ? (
            ...
            <WheelPickerWrapper className="w-56">
              <WheelPicker
                options={hourOptions}
                value={selHour}
                onValueChange={(v) => { setSelHour(v); commitTime(v, selMin, selMer) }}
                infinite
              />
              <WheelPicker
                options={minuteOptions}
                value={selMin}
                onValueChange={(v) => { setSelMin(v); commitTime(selHour, v, selMer) }}
                infinite
              />
              <WheelPicker
                options={meridiemOptions}
                value={selMer}
                onValueChange={(v) => { setSelMer(v); commitTime(selHour, selMin, v) }}
              />
            </WheelPickerWrapper>
          )}
        </div>
        <div className="border-t border-neutral-200 dark:border-neutral-800 pt-3 flex items-center justify-between">
          <button
            ...
            onClick={(e) => { e.stopPropagation(); setViewMode(currentView === "date" ? "time" : "date") }}
          ...
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-semibold text-neutral-400"><span className="text-[11px] font-semibold text-neutral-400">Time</span></span>
            <button
              type="button"
              onMouseDown={(e) => e.stopPropagation()}
              ...
              className={cn(
                ...
                includeTime ? "bg-[var(--brand)]" : "bg-neutral-300 dark:bg-neutral-600"
              )}
            >
              <span
                className={cn(
