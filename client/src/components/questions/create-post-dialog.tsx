...
export function CreatePostDialog() {
  ...
  if (isMobile) {
    return (
      <Drawer open={isOpen} onOpenChange={(val) => { if (!val) close(); }}>
        <DrawerContent className="h-[96vh] data-[vaul-drawer-direction=bottom]:max-h-[96vh] p-0 overflow-hidden bg-background rounded-t-3xl flex flex-col before:inset-x-0 before:bottom-0 before:top-0 before:rounded-t-3xl before:rounded-b-none before:bg-background before:border-x-0 before:border-b-0">
          <DrawerHeader className="px-5 pt-4 pb-3 flex flex-row items-center justify-between border-b border-neutral-150 dark:border-neutral-850">
            <DrawerTitle className="text-sm font-semibold text-neutral-850 dark:text-neutral-200">
              Create a Post
            </DrawerTitle>
            <Button size="sm" className="bg-[var(--brand)] hover:bg-[var(--brand-hover)] text-white rounded-lg text-xs h-8 px-4 border-none font-semibold cursor-pointer shrink-0"
              onClick={handleSubmit}
