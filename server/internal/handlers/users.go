...
func (h *APIHandler) UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	_, err := h.DB.Exec(ctx, "update users set bio = $1, avatar = $2, link = $3 where username = $4", profile.Bio, profile.Avatar, profile.Link, sub)
	if err != nil {
...
func (h *APIHandler) GetProfile(w http.ResponseWriter, r *http.Request) {
	...
	row := h.DB.QueryRow(ctx, "select username, email, COALESCE(bio, ''), COALESCE(avatar, ''), COALESCE(link, ''), posted, answered from users WHERE username = $1", sub)
